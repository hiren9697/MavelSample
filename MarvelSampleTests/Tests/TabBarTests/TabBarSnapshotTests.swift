...
final class TabBarSnapshotTests: XCTestCase {
    ...
    func test_tabBarController_withFirstTabSelected() {
        ...
        assertSnapshot(matching: sut, as: .image, named: "test_tabBarController_initializedFromFlowManagerAndWithFirstTabSelected", record: true)
    }
    
    func test_tabBarController_withSecondTabSelected() {
        sut.selectedIndex = 1
        assertSnapshot(matching: sut, as: .image, named: "test_tabBarController_initializedFromFlowManagerAndWithSecondTabSelected", record: true)
    }
    
    func test_tabBarController_withThirdTabSelected() {
        sut.selectedIndex = 2
        assertSnapshot(matching: sut, as: .image, named: "test_tabBarController_initializedFromFlowManagerAndWithThirdTabSelected", record: true)
    }
}
